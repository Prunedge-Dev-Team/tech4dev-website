...
const findCurrentIndex = (arrayName) =>
  arrayName.findIndex((e) => {
    const parentLeftStr = e.parentElement.style.left || "0px";
    const parentLeft = parseInt(parentLeftStr.split("px")[0]);
    return (
      e.offsetLeft + parentLeft >=
      document.querySelector(".partners-wrapper").scrollLeft
    );
  });

const findNext = () => {
  const sortedPartners = [...document.querySelectorAll(".partners")].sort(
    (a, b) => {
      if (
        parseInt(a.style.left.split("px")[0]) <
        parseInt(b.style.left.split("px")[0])
      ) {
        return -1;
      } else if (
        parseInt(a.style.left.split("px")[0]) >
...
const findPrev = () => {
  ...
  let currentIndex = findCurrentIndex([
    ...document.querySelectorAll(".current .partner"),
  ]);
...
window.addEventListener("DOMContentLoaded", () => {
  const clickListener = (buttonInstance, className, parentClassName) => {
    ...
    const currentIndex = items.findIndex(
      (e) => wrapper.scrollLeft <= e.offsetLeft,
    );
  ...
  const scrollWatcher = (className, parentClassName) => {
    ...
    if (
      Math.floor(wrapper.scrollWidth) ===
      Math.floor(wrapper.scrollLeft) + Math.floor(wrapper.offsetWidth)
    ) {
  ...
  document.querySelectorAll(".program-scroll-btn").forEach((e) => {
    e.addEventListener("click", () =>
      clickListener(e, "program", "programs-wrapper"),
    );
  });

  document
    .querySelector(".programs-wrapper")
    .addEventListener("scroll", () =>
      scrollWatcher("program-scroll-btn", "programs-wrapper"),
    );

  document.querySelectorAll(".testimonial-scroll-btn").forEach((e) => {
    e.addEventListener("click", () =>
      clickListener(e, "testimonial", "testimonials-wrapper"),
    );
  });
  ...
  document
    ...
    .addEventListener("scroll", () =>
      scrollWatcher("testimonial-scroll-btn", "testimonials-wrapper"),
    );
  ...
  carouselImages.forEach((e, i) => {
    document
      .querySelector(`.carousel-img:nth-of-type(${i + 2})`)
      .addEventListener("load", () => {
        carouselImages[i].loaded = true;
      });
    document.querySelector(`.carousel-img:nth-of-type(${i + 2})`).src =
  ...
  [...document.querySelectorAll(".carousel__change-btn")].forEach((e) => {
    e.addEventListener("click", () => {
      ...
      document
        .querySelector(`.carousel-img:nth-of-type(${activeCarousel})`)
        .classList.add("active");
    });
  });

  const iframeLinks = [
    "https://www.youtube.com/embed/snJClianGwA",
    "https://www.youtube.com/embed/4LASYETL58M",
    "https://www.youtube.com/embed/qcZKRUH2KAc",
  ];
...
const programImages = [...document.querySelectorAll(".program img")].map((el) =>
  transformElement(el, "./assets/images/placeholder.jpg"),
);
const testimonialImages = [
  ...document.querySelectorAll(".testimonial__image"),
].map((el) => transformElement(el, "./assets/images/female-avatar.png"));

const images = [
  ...programImages,
  transformElement(document.querySelector(".job-details__image"), ""),
  transformElement(
    document.querySelector(".concept_details__image img"),
...
const counterFunction = () => {
  ...
  const finalCount = ["201,382", "961,111", "82%", "82%", "58"];
  const speed = 200;
  counters.forEach((counter) => {
    const target = +counter.getAttribute("data-target");
    if (!counter.hasAttribute("data-target") || isNaN(target)) return;
    ...
    const updateCounter = (count) => {
      ...
      if (next < target) {
        counter.innerText = Math.round(next);
        setTimeout(() => updateCounter(next), 10);
      } else {
        if (target === 41079) counter.innerText = finalCount[0];
        else if (target === 10000000) counter.innerText = finalCount[1];
        else if (target === 36) counter.innerText = finalCount[2];
        else if (target === 1000) counter.innerText = finalCount[3];
        else if (target === 15) counter.innerText = finalCount[4];
      }
    };
